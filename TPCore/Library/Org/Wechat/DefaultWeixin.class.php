...
class DefaultWeixin extends WXMessage {
    ...
    public function processRequest($data) {
        ...
        if ($this->isTextMsg()) {
            switch ($input) {
                ...
                default:
                    //$this->text($input);
                    $this->loginIO($data->FromUserName);
                    break;
            }
        }
        // deal with geographical location
        elseif ($this->isLocationMsg()) {
            \Think\Log::record($data->FromUserName);
            $this->loginIO($data->FromUserName);
    ...
    private function fulinews() {
        ...
        $posts = array(
            array(
                'title' => '福利来了',
                'discription' => $text,
                'picurl' => 'http://mmsns.qpic.cn/mmsns/XWia2Xj7RZ8mhQaESostBicFaX2HjVBbJYKKCBk9PkuicKrSZdfNL7XAw/0111111',
     ...
     * 返回进入到系统的链接
     */
    private function loginIO($openid) {
        $text = '点此进入融易淘';
        $posts = array(
            array(
                'title' => '点此进入融易淘',
                'discription' => $text,
                'picurl' => 'https://ss1.baidu.com/6ONXsjip0QIZ8tyhnq/it/u=51553588,3796425299&fm=58',
                'url' => 'http://test.aleft.cn/?hellorongyigou0951='.$openid,
            )
        );
        $xml = $this->outputNews($posts);
        header('Content-Type: application/xml');
