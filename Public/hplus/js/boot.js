...
function executehtml(action,method,data,suc,err){
    $.ajax({url:action,data:data,type:(method==0?'get':'post'),cache:false,dataType:'html',beforeSend:function(XMLHttpRequest){_osajaxfree=false;_osloading = parent.layer.load(1,{shade: [0.5,'#000']});},success:suc,error:err,complete:function(XMLHttpRequest,textStatus){_osajaxfree=true;parent.layer.close(_osloading);}});
}
